...
int main()
{
	...
	float obj_dis = 0;//the ouput of the object distance
...
//	cin >> ReadMode;
	...
	while (1)
	{
		...
		{
			obj_dis = getDistance(frameL, frameDispfloat, result_vec[i], M_BF, D_INF, 2.0);//根据detection框坐标进行距离计算
		
		}
		imshow("frameL", frameL);
		imshow("frameD", frameDRGB);
		cout<<'object distance:'<<obj_dis<<endl;
